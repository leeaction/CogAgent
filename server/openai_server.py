"""
This script creates an OpenAI Server with transformers for the CogAgent model,
...
from transformers import AutoTokenizer, AutoModel, AutoModelForCausalLM, TextIteratorStreamer, QuantoConfig
...
def generate_stream_cogagent(model: AutoModel, tokenizer: AutoTokenizer, params: dict):
    ...
    streamer = TextIteratorStreamer(
        tokenizer=tokenizer, timeout=300, skip_prompt=True, skip_special_tokens=True
    )
    gen_kwargs = {
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="OpenAI Server for CogAgent")
    parser.add_argument(
    ...
    if args.quantization_type:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            args.model_path, torch_dtype="auto", trust_remote_code=True, device_map="auto", quantization_config=quantization_config
        ).eval()
    else:
